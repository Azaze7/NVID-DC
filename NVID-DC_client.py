...
# The body of the thread works as a server
...
# Gets all the packets of the client message
...
# Used to pull the key off the message
...
def incoming_message_parse(incoming):
    ...
    col_data = False
    currword = bytearray()
    # Gets file name from the combo of file name data and key
    ...
    # gets the key value that is encrypted using public key
    aes_key = rsa.decrypt(aes_key_enc, privk)
    
    # All use same iv - makes it easier
    iv = b'clienttoclientiv'
    # make the AES object
    aes_ob = AES.new(aes_key, AES.MODE_CFB, iv)
    
    # decrypt using key and all
    ...
    # plays the video or shows the image
    if '.mp4' in file_name or '.gif' in file_name:
...
# method used to send to server, just give message, port, and IP and it's good to go
...
# Used to send the start message with name, IP, and public key
def initial_message():
...
# gets the client list from the server
...
# Sends to other clients once a file has been selected.
def send_clients(file_selection, host_name, client_list):
    ...
    client_selection = ''
    client_pub_key = ''
    # gets the selected clients public key
    ...
    try:
        ...
        # makes the connects and then creates the message
        ...
        # creates packets and sends them all
        for packet in packets:
            client_socket.sendall(packet)
        
        # Sends a history message to the server
        history_message = 'updatehistory:' + hostname + ' with ' + ip + ' sent a file to  host ' + host_name + ' with IP ' + client_selection
...
# chunks the data into different packets.
...
# tells server leaving and then kills the program
def leave_and_kill():
...
# gets the different values from user interface and then calls methods
def submit_send():
    global comobox
...
# gets inputs from user interface and then updates name
def submit_name():
...
# shows all the history from the server
...
# shows the list of different clients and gets input from the user for which client to sent to
def popup_client():
    global comobox
    global client_list_server
...
# the interface face that runs the whole GUI
...
client_list_server = []
# client list and the server ip, port used, and host name
